...
func (conn *DbStruct) GetEmployee(id int64) (Entities.Employee, error) {
	...
	rows, err := conn.db.Query("SELECT * from employees WHERE employee_id = $1", id)
	if err != nil {
		return employee , err
	}
	defer rows.Close()
...
func (conn *DbStruct) PostEmployee(employee *Entities.Employee) error {
	_, err := conn.db.Exec("INSERT INTO employees(name, secondName, surname, photoUrl, hireDate, position, company_id) VALUES ($1, $2, $3, $4, $5, $6, $7)",
...
func (conn *DbStruct) PutEmployee(employee *Entities.Employee) error {
	_, err := conn.db.Exec("UPDATE employees set name = $1, secondName = $2, surname = $3, photoUrl = $4, hireDate = $5," +
...
func (conn *DbStruct) DeleteEmployee(id int64) error {
	_, err := conn.db.Exec("DELETE FROM employees WHERE employee_id = $1", id)
	return err
}
